...
while True:
    ...
    encodesCurFrame = elu(face_recognition.face_encodings(imgS, facesCurFrame))
    ...
    for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
        ...
        if facesDis[matchIndex] < 0.4:
            ...
            percentage = (1 - facesDis[matchIndex]) * 100                                                                                         + 25   
            identified = f"{name} ({percentage:.2f}%)"
            print(f"Match found: {name} ({percentage:.2f}%)")
            top, right, bottom, left = faceLoc
            top, right, bottom, left = top * 4, right * 4, bottom * 4, left * 4
            cv2.rectangle(img, (left, top), (right, bottom), (0, 255, 0), 2)
            cv2.putText(img, identified, (left + 6, top - 6), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
            markLocation(name)
